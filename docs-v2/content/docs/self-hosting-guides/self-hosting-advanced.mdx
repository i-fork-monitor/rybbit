...
./setup.sh tracking.example.com --mapbox-token YOUR_MAPBOX_TOKEN --backend-port 8080 --client-port 8081
```
...
MAPBOX_TOKEN=your_mapbox_token  # Optional, only added if --mapbox-token is provided
```

Optional variables that can be added:

```bash
# Mapbox token for map visualizations (recommended)
